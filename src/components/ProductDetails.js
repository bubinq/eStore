...
export const ProductDetails = () => {
  ...
  // getCurrent Product
  useEffect(() => {
    getProduct(finalId).then((res) => {
      setLocalProduct(res.data());
    });
    if (commentAdded) {
      getComments(finalId).then((res) => {
        setNumPages(
          new Array(Math.ceil(res.size / 8)).fill({ isSelected: false })
        );
      });
    }
  ...
  useEffect(() => {
    if (commentAdded) {
      getFirstComments(finalId).then((res) => {
        setProductComments(
  ...
  const goToPage = (ev) => {
    ...
    setNumPages((recentPages) =>
      recentPages.map((page, idx) =>
        idx === currentPage - 1 ? { isSelected: true } : { isSelected: false }
      )
    );
    let page = 0;
    if (currentPage !== 1) {
    ...
    getComments(finalId)
      ...
      .then((latestDoc) => {
        sortComments(finalId, latestDoc).then((res) => {
          setProductComments(
            res.docs.map((doc) => ({ ...doc.data(), id: doc.id }))
          );
        });
      ...
      <>
        ...
        <div className={styles.mainWrapper}>
          <div className={styles.productWrapper}>
            ...
            <div className={styles.description}>
              ...
              <span className={styles.price}>
                {Number(localProduct.price).toFixed(2)}$
              </span>
          ...
          <div className={styles.formWrapper}>
            ...
            {authUser && (
              <div className={styles.reviewWrapper}>
                ...
                >
                  {isWriting ? "Hide section" : "Write a comment"}
                </button>
              </div>
            )}
                      ...
                      {[...Array(5)].map((star, idx) => (
                        ...
                        >
                          <FaStar
                            ...
                            style={{
                              color: idx < hover ? "rgb(255, 232, 58)" : "gray",
                            }}
                          ></FaStar>
                          <input
                            type="radio"
          ...
          {productComments.map((comment) => (
            <Comment key={comment.createdAt} comment={comment}></Comment>
          ))}
